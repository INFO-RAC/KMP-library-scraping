...
for tip in range(1, 8):
    ...
    for element in div_library:
        ...
        # Aggiungi i dati al DataFrame - FIXED: use pd.concat with a list containing the DataFrame and a new row
        library = pd.concat([library, pd.DataFrame({'handle': [a_handle], 'title': [a_title], 'date': [a_date], 'document_type': ['pubblication']})], ignore_index=True)
...
for index, row in df_planbleau.iterrows():
    ...
    for element_sub in div_links:
        ...
        library = pd.concat([library, pd.DataFrame({'handle': [a_handle], 'title': [a_title], 'date': [a_date], 'document_type': ['pubblication'], 'download': [a_link]})], ignore_index=True)


library.to_excel('Plan_bleau_library_scraping_complete_1_7.xlsx', index=False)
